...
Telegram `callback_data` is one bot-wide namespace. Any extension that creates inline buttons for a bot shared with `pa-telegram` must use namespaced callback data.
...
## pa-telegram fallback

If `pa-telegram` receives callback data that is not owned by its built-in prefixes and no built-in handler consumes it, it forwards the click to Pi as:
...
[Telegram Extension Sections](./sections.md) are a higher-level UI contract over this namespace rule. A section owns a canonical extension identity such as `@llblab/pi-telegram-explorer`, but its Telegram `callback_data` should use the `pa-telegram` owned `section:` prefix plus a compact token, because Telegram limits callback payloads to 64 bytes.
